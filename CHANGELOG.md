...
- **Sprint 33 — Fontes de design (JetBrains Mono + Fira Code) + higiene Flatpak do Boxy SVG**: novo `scripts/instalar_fontes_design.sh` instala as fontes de design **JetBrains Mono** (16 TTF da família principal — Thin→ExtraBold + itálicos) e **Fira Code** (6 TTF — Light/Regular/Retina/Medium/SemiBold/Bold) em `~/.local/share/fonts/{JetBrainsMono,FiraCode}/`, com o `OFL.txt` de cada uma (ambas SIL OFL-1.1, redistribuição permitida). As `.ttf` são **versionadas** em `src/fontes-design/` (bundle offline/reprodutível — o install não baixa nada). Idempotente (`cmp -s` por arquivo; `fc-cache` só quando algo muda); sem sudo. Corrige na raiz o bug em que a exportação PNG de um `.svg` no **Boxy SVG** perdia o texto vivo (`PROTOCOLO OUROBOROS` sumia) por ausência das fontes — o Flatpak do Boxy já lê `~/.local/share/fonts`, faltavam as fontes no host. **Higiene Flatpak** não-fatal: quando o Boxy (`com.boxy_svg.BoxySVG`) está instalado e as fontes mudaram, limpa o cache de fontconfig do sandbox (`~/.var/app/com.boxy_svg.BoxySVG/cache/fontconfig` — regenera no próximo start) e avisa para reiniciar o app; prova opcional via `flatpak run --command=fc-match`. Verificação final por `fc-match` no host (JetBrains Mono/Fira Code resolvem para a fonte, não fallback). Wired em `install.sh`: nova flag `--fontes-design` (fase user, não-fatal) **incluída em `--all`** (e portanto em `--bootstrap`). Reversível via `scripts/desinstalar_fontes_design.sh`.
...
- **Sprint 36 (parte 2) — Build reprodutível e heal que confere a arte instalada**: duas rodadas de `build.sh` sobre o mesmo source produziam **784 de 4414 arquivos** com bytes diferentes — o `convert` carimba a hora da conversão dentro do PNG (pixels idênticos, metadata diferente; o `rsvg-convert` das fontes SVG já era determinístico). Corrigido com `-define png:exclude-chunk=date,tIME` em `redimensionar_png` e no ramo `magick` de `converter_svg`; verificado byte-a-byte em duas rodadas consecutivas. Sem isso, nenhuma verificação por hash entre `dist/` e o tema instalado distingue "arte regrediu" de "buildou de novo". Com o build determinístico, `scripts/reaplicar_tema.sh` ganha a **seção 1.5**: ressincroniza `dist/icons/<tema>/` para `~/.local/share/icons/<tema>/` via `rsync -rlc` (checksum) nos quatro temas, reportando quantos arquivos divergiam. Antes o heal só conferia a **existência** do diretório e regenerava cache/`gsettings` — arte regredida no destino sobrevivia a qualquer número de heals, e cache regenerado a partir de arte errada continua errado. Detalhes: `-l` é obrigatório (os `@2x` dos upstreams são symlinks e o `rsync` sem ele imprime `skipping non-regular file` em stdout, inflando a contagem para 9438/13342 a cada rodada); `--exclude=icon-theme.cache` (gerado localmente pela seção 8); sem `--delete` (mesmo contrato do `install.sh`). Verificado com regressão injetada: detectada, curada, e a rodada seguinte reporta sincronia sem escrever. A seção 1.5 usa `--delete` com duas guardas: `validar_path_destrutivo` (allowlist de `lib/common.sh`) e razão dist/destino — se o `dist/` tem menos de 90% dos arquivos do destino, é build parcial e a sincronização roda **sem** deletar (testado removendo 10 dos 49 arquivos do `dist/` do `Dracula-Cursor`: avisou e preservou os 49 instalados). Removeu os **32 órfãos** que sobravam no `Dracula-Icones` (`antigravity*`, `com.obsproject.Studio`, `guvcview`, mimetypes `.svg` de vídeo da SPRINT 22, `protocolo-ouroboros.svg` copiado à mão), respaldados antes em `~/.cache/dracula_os_backup/icones_orfaos_20260818-225250/` (tar + manifest sha256, 32/32 conferidos); destino e `dist/` passam a ter as mesmas 4413 entradas. Sem backup a cada rodada de propósito: o conteúdo é regenerável por `./build.sh && ./install.sh --user`.
- **Sprint 36 — Ícone do Protocolo Ouroboros voltou a ser a marca atual**: `src/icons/projects/protocolo-ouroboros.png` guardava o brasão antigo do projeto desde `30f826c9` (2026-04-16) e nunca foi atualizado quando a marca do app mudou. Inofensivo enquanto o `mapping.json` não cobria o nome; a SPRINT 35 (`32d35ec2`) mapeou `protocolo-ouroboros` + alias `ouroboros` para essa arte, o `build.sh` materializou os PNGs em `Dracula-Icones/*/apps/` e — como o tema ativo é varrido antes do `hicolor` — a arte antiga passou a **sombrear** a marca que o `install.sh` do próprio app instala no `hicolor`. O app tinha escolhido o nome `protocolo-ouroboros` justamente por ser um nome que nenhum tema define (documentado no `install.sh` dele, linhas 228-243); a SPRINT 35 anulou a proteção ao definir exatamente esse nome. Correção: a arte-mestre do tema passa a ser cópia byte-a-byte de `protocolo-ouroboros/assets/icon.png`, a mesma fonte que o instalador do app usa (e **não** o `app-icon.png`, cujo anel embutido em disco opaco colapsa por antialiasing no tamanho do lançador). Nome do arquivo-fonte mantido em `.png` de propósito: migrar para `.svg` deixaria o `.png` antigo órfão em `scalable/apps/` do tema instalado, e `.png` vence `.svg` na ordem de extensões do lookup freedesktop. Detalhes em `docs/sprints/SPRINT_36_ICONE_OUROBOROS_ARTE_ANTIGA.md`.
